...
const QuizSection = () => {
  ...
  const handleSubmit = () => {
    if (selectedAnswer === null) {
      ...
      alert(`Good job! You selected: ${selectedAnswer}`);
    }
  };
  ...
  return (
    <div className="mt-6 p-4 bg-pink-50 rounded-lg shadow-lg">
      <h2 className="text-lg font-bold text-purple-600">Quiz Time!</h2>
      <div className="mb-4">
        <p className="font-semibold text-gray-800">{quizQuestion.question}</p>
        <ul className="mt-2">
          {quizQuestion.options.map((option, idx) => (
            <li key={idx} className="ml-4 mb-2">
              ...
              />
              <label htmlFor={`option-${idx}`} className="ml-2 text-gray-700">{option}</label>
            </li>
          ))}
        </ul>
        <button 
          onClick={handleSubmit} 
          className="mt-2 px-6 py-2 bg-blue-500 text-white rounded-full font-bold hover:bg-blue-400 transition-colors"
        >
          Submit Answer
        </button>
      </div>
